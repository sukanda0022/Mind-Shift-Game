...
function handleBackgroundTime() {
    ...
    if (lastExit && lastExit !== "undefined") {
        ...
        if (diffSeconds > 0) {
            ...
            // ใช้ Logic เดียวกับลูปเกม: ถ้าสถานะคือสลับแอปจริงถึงจะหัก
...
// --- [แก้ไขแล้ว] ลูปเกมหลัก: ปรับให้เช็คสถานะรวมเพื่อความแม่นยำ ---
    ...
    gameInterval = setInterval(async () => {
        ...
        if (timeLeft > 0) {
            ...
            if (!isBreakMode) {
                ...
                // 🛠️ ตรวจสอบสถานะจริง: หักพลังงานเฉพาะเมื่อ "ไม่อยู่หน้าจอ" และ "ยืนยันว่าสลับแอป"
                if (isSleeping && isActuallySwitched) {
                    ...
                    // กรณีปกติ หรือ ปิดจอดับ (isActuallySwitched เป็น false) -> นับโฟกัสให้ปกติ
                    totalFocusSeconds++;
...
// ✨ [ส่วนแก้ไขสำคัญ: แยกจอดับ VS สลับแอป ให้รองรับหลายรุ่น]
...
document.addEventListener('visibilitychange', () => {
    if (document.hidden) {
        ...
        // 🛠️ ปรับเป็น 1500ms (1.5 วินาที) เพื่อความชัวร์ในมือถือรุ่นที่ประมวลผลช้า
        // ถ้าวินาทีที่หายไปน้อยกว่า 1.5 วิ แสดงว่า "ตั้งใจสลับแอป" (กด Home/ปุ่มสลับ)
        if (timeSinceBlur < 1500) { 
            isActuallySwitched = true; 
            tabSwitchCount++;
            updateOnlineStatus("สลับแอป");
        } else {
            // ถ้าหายไปช้ากว่านั้น แสดงว่าเป็นพฤติกรรม "จอดับเอง" หรือ "กดปุ่ม Power"
            isActuallySwitched = false;
