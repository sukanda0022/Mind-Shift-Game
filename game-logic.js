...
// --- [แก้ไขแล้ว] ลูปเกมหลัก: ปรับ Logic ให้ปิดจอแล้วพลังงานไม่ลด ---
function startGameLoop() {
    ...
    gameInterval = setInterval(async () => {
        ...
        if (timeLeft > 0) {
            timeLeft--;
            if (!isBreakMode) {
                // 🔴 กรณีสลับแอปจริง (แอบหนีไปแอปอื่น)
                ...
                // 🟢 กรณีหน้าจอปกติ หรือ ปิดจอแบบค้างเว็บไว้ (ไม่ได้สลับแอป)
                else {
                    totalFocusSeconds++; // ยังคงนับวินาทีที่โฟกัสให้
                    if (periodEnergy < 100) periodEnergy += 0.3; // คืนพลังงานเล็กน้อย
                }
            }
            updateUI();
        } else {
...
document.addEventListener('visibilitychange', () => {
    if (document.hidden) {
        ...
        // แยกแยะ: ถ้าหายไปเร็วมากหลัง Blur แสดงว่ากดปุ่ม Home/สลับแอป
        if (timeSinceBlur < 600) { 
            isActuallySwitched = true; 
            tabSwitchCount++;
            updateOnlineStatus("สลับแอป");
        } else {
            // ถ้าหายไปช้ากว่านั้น มักเกิดจากจอดับเองหรือกดปุ่ม Power
            isActuallySwitched = false;
