...
function handleBackgroundTime() {
    ...
    if (lastExit && lastExit !== "undefined") {
        ...
        if (diffSeconds > 0) {
            ...
            // ✨ แก้ไขจุดนี้: ถ้าไม่ได้ตั้งใจสลับแอป (คือปิดจอ) ให้คืนพลังงานและนับวินาทีโฟกัส
...
// --- [แก้ไขแล้ว] ลูปเกมหลัก: บังคับไม่หักคะแนนถ้าสถานะคือปิดจอ ---
    ...
    gameInterval = setInterval(async () => {
        ...
        if (timeLeft > 0) {
            ...
            if (!isBreakMode) {
                ...
                // ✨ แก้ไขจุดสำคัญ: บังคับให้หักคะแนนเฉพาะเมื่อ "สลับแอป" เท่านั้น
                // ถ้าสถานะเป็น "ออนไลน์ (ปิดจอ)" หรือ "online" จะไม่มีการหักพลังงาน
                if (isSleeping && isActuallySwitched) {
                    ...
                    // กรณีเปิดจอปกติ หรือ ปิดจอดับทิ้งไว้บนเว็บเดิม
                    totalFocusSeconds++;
...
// ✨ [ส่วนแก้ไขการแยกแยะ: เน้นความแม่นยำสูงขึ้น]
...
document.addEventListener('visibilitychange', () => {
    if (document.hidden) {
        ...
        // 🛠️ มือถือส่วนใหญ่เมื่อปิดจอ (Power Button) ค่า Delay ระหว่าง Blur และ Hidden จะสูง
        // เราตั้งเกณฑ์ไว้ที่ 1000ms (1 วินาที) ถ้าเร็วกว่านี้คือการกดสลับแอป/Home
        if (timeSinceBlur < 1000) { 
            isActuallySwitched = true; 
            tabSwitchCount++;
            updateOnlineStatus("สลับแอป");
        } else {
            isActuallySwitched = false; // ปิดหน้าจอเฉยๆ
